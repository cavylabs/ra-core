...
var useGetManyReference = function (resource, target, id, pagination, sort, filter, referencingResource, options) {
    ...
    var _a = useQueryWithStore_1["default"].apply(void 0, [{
        type: 'getManyReference',
        resource: resource,
        payload: { target: target, id: id, pagination: pagination, sort: sort, filter: filter }
    }, {}].concat(options, [relatedTo, action, crudGetManyReference_1.CRUD_GET_MANY_REFERENCE])), ids = _a.data, total = _a.total, error = _a.error, loading = _a.loading, loaded = _a.loaded;
...
var selectData = function (reference, relatedTo) { return function (state) {
    return oneToMany_1.getReferences(state, reference, relatedTo);
}; };
